...
+function($) {
	...
	var Validator = function(element, options) {
		...
		this.$element.on('input.bs.validator blur.bs.validator', ':input', $
	...
	Validator.prototype.validateInput = function(e) {
		...
		this.$element.trigger($.Event('validated.bs.validator', {
			relatedTarget : $el[0]
		}))
	}
	...
	Validator.prototype.runValidators = function($el) {
		...
		$.each(Validator.VALIDATORS, $.proxy(function(key, validator) {
			if (($el.data(key) || key == 'native')
				...
				var error = $el.data(key + '-error') || $el.data('error')
	...
	Validator.prototype.showErrors = function($el) {
		function callback() {
			...
			$block.data('bs.originalContent') === undefined
					&& $block.data('bs.originalContent', $block.html())
		...
		if (this.options.delay) {
			...
			$el.data('bs.timeout', window.setTimeout(callback,
					this.options.delay))
		} else
			callback()
	}
