...
/**
 * A module finder which locates module specifications which are stored in a
 * local module repository on the filesystem, which uses {@code module.xml}
 * descriptors.
 * 
 * @author <a href="mailto:david.lloyd@redhat.com">David M. Lloyd</a>
 */
public final class HwTxModuleFinder implements ModuleFinder {

	private final File[] repoRoots;
	private final PathFilter pathFilter;
	private final AccessControlContext accessControlContext;

	private HwTxModuleFinder(final File[] repoRoots, final PathFilter pathFilter, final boolean cloneRoots) {
	 ...
	 */
	public HwTxModuleFinder(final File[] repoRoots, final PathFilter pathFilter) {
	 ...
	 *            the repository roots to use
	 */
	public HwTxModuleFinder(final File[] repoRoots) {
	 ...
	 * module repository roots.
	 * 
	 * @param supportLayersAndAddOns
	 *            {@code true} if the identified module repository roots should
	 *            be checked for an internal structure of child "layer" and
	 *            "add-on" directories that may also be treated as module roots
	 *            lower in precedence than the parent root. Any "layers"
	 *            subdirectories whose names are specified in a
	 *            {@code layers.conf} file found in the module repository root
	 *            will be added in the precedence of order specified in the
	 *            {@code layers.conf} file; all "add-on" subdirectories will be
	 *            added at a lower precedence than all "layers" and with no
	 *            guaranteed precedence order between them. If {@code false} no
	 *            check for "layer" and "add-on" directories will be performed.
	 * 
	 */
	public HwTxModuleFinder(boolean supportLayersAndAddOns) {
	...
	public ModuleSpec findModule(final ModuleIdentifier identifier, final ModuleLoader delegateLoader) throws ModuleLoadException {
		...
		ModuleManager moduleManager = HwTx.getModuleManager();
		
		ModuleSpec moduleSpec = moduleManager.getModuleSpecs().get(identifier);
		if (moduleSpec != null) {
			return moduleSpec;
		}
	...
	/**
	 * Parse a {@code module.xml} file and return the corresponding module
	 * specification.
	 * 
	 * @param identifier
	 ...
	 * @throws IOException
	 *             if reading the module file failed
	 * @throws ModuleLoadException
	 *             if creating the module specification failed (e.g. due to a
	 *             parse error)
	 */
	public static ModuleSpec parseModuleXmlFile(final ModuleIdentifier identifier, final ModuleLoader delegateLoader, final File... roots)
