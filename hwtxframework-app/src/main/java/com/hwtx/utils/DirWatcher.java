...
public final class DirWatcher {
	...
	public DirWatcher(Path pathToMonitor, boolean watchSubDir, WatchEventHandler eventHandler) {
	...
	private void register(Path pathToMonitor, int level) throws IOException {
		WatchKey key = pathToMonitor.register(ws, ENTRY_CREATE, ENTRY_DELETE, ENTRY_MODIFY);
		...
		if (debug) {
			...
			if (prev == null) {
				...
				if (!pathToMonitor.equals(prev.path)) {
					System.out.format("-->\tupdate: %s -> %s\tregisted level=%d\n", prev, pathToMonitor, level);
				}
			}
		}
		KeyState prev = keyMap.get(key);
	 ...
	 * @throws IOException
	 */
	private void registerAll(final Path curPath, final int maxLevel) throws IOException {
	 ...
	 * @throws IOException
	 */
	private void registerAll(final Path curPath, final int curLevel, final int maxLevel) throws IOException {
		...
		Files.walkFileTree(curPath, new SimpleFileVisitor<Path>() {
			...
			public FileVisitResult preVisitDirectory(Path dir, BasicFileAttributes attrs) throws IOException {
				...
				if (excludeSet.contains(dir.toFile().getPath())) {
			...
			public void run() {
				while (true) {
					...
					for (WatchEvent<?> event : key.pollEvents()) {
						...
						if (excludeSet.contains(child.toFile().getPath())) {
							continue;
						}

						// 增加到事件队列中
						KeyState eventState = new KeyState(child, stateOrigin.level);// 不使用level字段
						...
						// 事件的信息
						if (debug)
							System.out.format("%d:%s: %s\n", keyEventCount, event.kind().name(), child);
						...
						if (watchSubDir && (kind == ENTRY_CREATE)) {
							try {
								if (Files.isDirectory(child, LinkOption.NOFOLLOW_LINKS)) {
									if (debug)
										System.out.println(key + "\t原始key," + keyMap.get(key));
									registerAll(child, stateOrigin.level + 1, maxLevel);
								}
							} catch (IOException e) {
								// e.printStackTrace();
							}
					...
					if (eventList.size() == 1) {
						...
						for (int i = 0; i < size; i++) {
							...
							if (i + 1 < size) {
								...
								// 重命名，先删除再创建
								if (eventStart.opType == OperationType.Delete && eventEnd.opType == OperationType.Create) {
	...
	private void processEvent(WatchKey key, KeyState event) {
		...
		if (event.opType != OperationType.Modify || !event.path.equals(keyState.exPath) || event.opTime - keyState.opTime > 200) {// 同一文件的两个modify时间过短，过滤掉一个（之前也有过滤，但中间可能插入了文件夹的modify事件）；注意keyState中exPath是事件记录，path是监控的文件夹路径

			if (event.opType != OperationType.Modify || !Files.isDirectory(event.path, LinkOption.NOFOLLOW_LINKS)) {// 过滤文件夹的modify事件，毕竟文件夹modify是因为文件夹下的文件的改变，已经有对应事件了！！！当然，理论上应该再检测一下文件夹是否处于监控列表中，懒得写了，反正都打算用jnotify了！！！
				if (debug)
					System.out.println("key=" + key.hashCode() + "\t" + event);
				eventHandler.handleEvent(event);
			}
		}
		keyState.exPath = event.opType == OperationType.Rename ? event.exPath : event.path;// 作为上一步操作的文件进行记录
