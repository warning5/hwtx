...
public final class JFinal {
	...
	private void initHandler() {
		...
		String handlerClazz = JFinalConfig.getProperty("root.actionHandler", ActionHandler.class.getName());
		try {
			RootActionHandlerFactory factory = (RootActionHandlerFactory) Class.forName(handlerClazz).newInstance();
			Handler actionHandler = factory.createHandler(constants, actionMapping);
			handler = HandlerFactory.getHandler(Config.getHandlers().getHandlerList(), actionHandler);
		} catch (Exception e) {
			System.out.println("--------create handler " + handlerClazz + " failure.----------------");
		}
	}
