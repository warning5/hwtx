...
/**
 * A classloader which can delegate to multiple other classloaders without risk
 * of deadlock. A concurrent class loader should only ever be delegated to by
 * another concurrent class loader; however a concurrent class loader <i>may</i>
 * delegate to a standard hierarchical class loader. In other words, holding a
 * lock on another class loader while invoking a method on this class loader may
 * cause an unexpected deadlock.
 * 
...
public abstract class ConcurrentClassLoader extends ClassLoader {
	...
	static {
		...
		/*
		 * This resolves a known deadlock that can occur if one thread is in the
		 * process of defining a package as part of defining a class, and
		 * another thread is defining the system package that can result in
		 * loading a class. One holds the Package.pkgs lock and one holds the
		 * Classloader lock.
		 */
		Package.getPackages();
	...
	/**
	 * Construct a new instance with the given parent class loader, which must
	 * be a concurrent class loader, or {@code null} to create a root concurrent
	 * class loader.
	 * 
	 * @param parent
	...
	/**
	 * Loads the class with the specified binary name. Equivalent to calling
	 * {@link #loadClass(String, boolean) loadClass(className, false)}.
	 * 
	 * @param className
	 ...
	 *            The binary name of the class
	 * @param resolve
	 *            {@code true} if the class should be linked after loading
	 * @return the resulting {@code Class} instance
	 */
	@Override
	...
	/**
	 * Same as {@link #loadClass(String)}, except only exported classes will be
	 * considered.
	 * 
	 * @param className
	 *            the class name
	...
	/**
	 * Same as {@link #loadClass(String,boolean)}, except only exported classes
	 * will be considered.
	 * 
	 * @param className
	 *            the class name
	...
	/**
	 * Find a class, possibly delegating to other loader(s). This method should
	 * <b>never</b> synchronize across a delegation method call of any sort. The
	 * default implementation always throws {@code ClassNotFoundException}.
	 * <p>
	 * If a class is to be defined by this method, it should be done via one of
	 * the atomic {@code defineOrLoadClass} methods rather than
	 * {@code defineClass()} in order to avoid spurious exceptions.
	 * 
	 * @param className
	 *            the class name
	 * @param exportsOnly
	 *            {@code true} if only exported classes should be considered
	 * @param resolve
	 *            {@code true} if the class should be linked after loading
	 * @return the class
	 * @throws ClassNotFoundException
	...
	/**
	 * Atomically define or load the named class. If the class is already
	 * defined, the existing class is returned.
	 * 
	 * @param className
	 *            the class name to define or load
	 * @param bytes
	 *            the bytes to use to define the class
	 * @param off
	 *            the offset into the byte array at which the class bytes begin
	 * @param len
	 ...
	 * {@code '/'}-separated path name that identifies the resource. If the
	 * resource name starts with {@code "java/"} then the parent class loader is
	 * used. Otherwise, this method delegates to
	 * {@link #findResource(String, boolean)}.
	 * 
	 * @param name
	 *            the name of the resource
	 * @return the resource URL, or {@code null} if no such resource exists or
	 *         the invoker does not have adequate permission to access it
	 */
	public final URL getResource(final String name) {
		...
		for (String s : Module.systemPaths) {
			if (name.startsWith(s)) {
				// Whatever loads jboss-modules.jar should have it's classes
				// accessible.
				// This could even be the bootclasspath, in which case CL is
				// null, and we prefer the system CL
	 ...
	 * @param name
	 *            the resource name
	 * @return an enumeration over all the resource URLs; if no resources could
	 *         be found, the enumeration will be empty
	 * @throws IOException
	 ...
	 *            the resource name
	 * @param exportsOnly
	 *            {@code true} to consider only exported resources or
	 *            {@code false} to consider all resources
	...
	/**
	 * Never used. {@link ClassLoader#getResource(String)} and related methods
	 * can cause a loop condition when this method is implemented; use
	 * {@link #findResource(String, boolean)} instead.
	 * 
	 * @param name
	 *            ignored
	...
	protected final URL findResource(final String name) {
		// Always return null so that we don't go into a loop from
		// super.getResource*().
		return null;
	}
	...
	/**
	 * Never used. {@link ClassLoader#getResources(String)} and related methods
	 * can cause a loop condition when this method is implemented; use
	 * {@link #findResources(String, boolean)} instead. By default, returns an
	 * empty enumeration.
	 * 
	 * @param name
	 *            ignored
	...
	/**
	 * Finds the resource with the given name and exported status, returning the
	 * resource content as a stream.
	 * 
	 * @param name
	 *            the resource name
	 * @param exportsOnly
	...
	/**
	 * Returns an input stream for reading the specified resource. If the
	 * resource starts with {@code "java/"}, then this method delegates to the
	 * parent class loader. Otherwise, this method delegates to
	 * {@link #findResourceAsStream(String, boolean)}.
	 * 
	 * @param name
	 *            the resource name
	 * @return the resource stream, or {@code null} if the resource is not found
	...
	/**
	 * Perform a class load operation. If the class is in the package or a
	 * subpackage of a package in the system packages list, the parent class
	 * loader is used to load the class. Otherwise, this method checks to see if
	 * the class loader object is locked; if so, it unlocks it and submits the
	 * request to the class loader thread. Otherwise, it will load the class
	 * itself by delegating to {@link #findClass(String, boolean, boolean)}.
	 * 
	 * @param className
	 *            the class name
	 * @param exportsOnly
	 ...
	 *            {@code false} to consider all resources
	 * @param resolve
	 *            {@code true} to resolve the loaded class
	 * @return the class returned by
	 *         {@link #findClass(String, boolean, boolean)}
	 * @throws ClassNotFoundException
	 *             if {@link #findClass(String, boolean, boolean)} throws this
	 *             exception
	 */
	...
	/**
	 * Perform a class load operation. This method checks to see if the class
	 * loader object is locked; if so, it unlocks it and submits the request to
	 * the class loader thread. Otherwise, it will load the class itself by
	 * delegating to {@link #findClass(String, boolean, boolean)}.
	 * <p>
	 * If the {@code jboss.modules.unsafe-locks} system property is set to
	 * {@code true}, then rather than using the class loading thread, the lock
	 * is forcibly broken and the load retried.
	 * 
	 * @param className
	 *            the class name
	 * @param exportsOnly
	...
	private Class<?> performLoadClassChecked(final String className, final boolean exportsOnly, final boolean resolve) throws ClassNotFoundException {
		if (SAFE_JDK) {
			...
			if (Thread.currentThread() != LoaderThreadHolder.LOADER_THREAD) {
				// Only the classloader thread may take this lock; use a
				// condition to relinquish it
		...
		// no deadlock risk! Either the lock isn't held, or we're inside the
		// class loader thread.
		return performLoadClassUnchecked(className, exportsOnly, resolve);
	}
	 ...
	 * @param name
	 *            the package name
	 * @return the package, or {@code null} if no such package is visible to
	 *         this class loader
	 */
	...
	/**
	 * Perform the actual work to load a package which is visible to this class
	 * loader. By default, uses a simple parent-first delegation strategy.
	 * 
	 * @param name
	 *            the package name
	 * @return the package, or {@code null} if no such package is visible to
	 ...
	 * @param name
	 *            the package name
	 * @return the package or {@code null} if no such package is defined by this
	 *         class loader
	 */
	...
	/**
	 * Defines a package by name in this <tt>ConcurrentClassLoader</tt>. If the
	 * package was already defined, the existing package is returned instead.
	 * 
	 * @param name
	 *            the package name
	 * @param specTitle
	 ...
	 * @param sealBase
	 *            if not {@code null}, then this package is sealed with respect
	 *            to the given code source URL
	 * 
	 * @return the newly defined package, or the existing one if one was already
	 *         defined
	 */
	protected Package definePackage(final String name, final String specTitle, final String specVersion, final String specVendor, final String implTitle, final String implVersion,
