	...
	public void config() {
		...
		for (Class controller : controllerClasses) {
			...
			if (controllerBind == null) {
				...
				if (StrKit.isBlank(cKey)) {
					throw new RuntimeException("can't fing any variable of " + controllerBind.controllerKey());
				}
				if (StrKit.isBlank(controllerBind.viewPath())) {
					this.add(cKey, controller);
					logger.debug("routes.add(" + controllerBind.controllerKey() + ", " + controller.getName() + ")");
				} else {
					this.add(cKey, controller, controllerBind.viewPath());
